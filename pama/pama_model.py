...
from pama.histo_loss import RGBuvHistBlock, calc_histogram_loss
from pama.util import (
    calc_ss_loss,
    calc_remd_loss,
    calc_moment_loss,
    calc_mse_loss,
...
class PAMANet(nn.Module):
    def __init__(self, args):
        super(PAMANet, self).__init__()
        self.args = args
        self.vgg = vgg19[:44]
        self.vgg.load_state_dict(torch.load(f'{args.checkpoints}/encoder.pth', map_location='cpu'), strict=False)
        ...
        if args.pretrained == True:
            self.align1.load_state_dict(torch.load(f'{args.checkpoints}/PAMA1.pth', map_location='cpu'), strict=True)
            self.align2.load_state_dict(torch.load(f'{args.checkpoints}/PAMA2.pth', map_location='cpu'), strict=True)
            self.align3.load_state_dict(torch.load(f'{args.checkpoints}/PAMA3.pth', map_location='cpu'), strict=True)
            self.decoder.load_state_dict(torch.load(f'{args.checkpoints}/decoder.pth', map_location='cpu'), strict=False)
    ...
    def save_ckpts(self):
        torch.save(self.align1.state_dict(), f"{self.args.checkpoints}/PAMA1.pth")
        torch.save(self.align2.state_dict(), f"{self.args.checkpoints}/PAMA2.pth")
        torch.save(self.align3.state_dict(), f"{self.args.checkpoints}/PAMA3.pth")
        torch.save(self.decoder.state_dict(), f"{self.args.checkpoints}/decoder.pth")  
